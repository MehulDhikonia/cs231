...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that perorms an affine transform followed by batch norm
    followed by a ReLU
    ...
    norm, nm_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(norm)
    cache = (fc_cache, nm_cache, relu_cache)
    return out, cache
...
def affine_norm_relu_backward(dout, cache):
    ...
    dnm, dgamma, dbeta = batchnorm_backward_alt(da, nm_cache)
    dx, dw, db = affine_backward(dnm, fc_cache)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss, dscores = softmax_loss(out2, y)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #Initialize list ot store outputs and cache
        outcache = []
        ...
        for i in range(num_layers):
            if i < num_layers - 1:
                ...
                if not self.use_batchnorm:
                    ...
                    tmp['out' + idx], tmp['cache' + idx] = \
                                                  affine_norm_relu_forward(x_reshaped, \
                                                               self.params['W' + idx], \
                                                               self.params['b' + idx], \
            ...
            #Final Layer forward pass (affine - softmax)
                idx = str(i+1)
                tmp = {}
                tmp['out' + idx], tmp['cache' + idx] = affine_forward(x_reshaped, \
                                                          self.params['W' + idx], \
                                                          self.params['b' + idx])
                outcache.append(tmp)
        ...
        for i in range(num_layers):
            idx = str(i+1)
            reg_loss += 0.5*self.reg * np.sum(self.params['W' + idx]**2)
        loss += reg_loss

        for i in range(num_layers):
            idx = num_layers - i - 1
            cache = outcache[-(i+1)]['cache' + str(idx+1)]
            if i == 0:
                ...
                if not self.use_batchnorm:
                    ...
                    grads['W' + str(idx+1)] += \
                                              self.reg*self.params['W' + str(idx+1)]
                else:
                    dout, grads['W' + str(idx+1)], grads['b' + str(idx+1)], dgamma, dbeta \
                                            = affine_norm_relu_backward(dout, cache)
                    grads['W' + str(idx+1)] += \
                                              self.reg*self.params['W' + str(idx+1)]
